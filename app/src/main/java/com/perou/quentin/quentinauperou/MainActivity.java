...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mWeatherInput = (Spinner) findViewById(R.id.weather);
        mWeatherIcon = (ImageView) findViewById(R.id.imageView);

        List<String> weatherValues;
        ArrayAdapter<String> weatherAdapter;

        weatherValues = new ArrayList<String>();
        for (WeatherValues wv : WeatherValues.values()) {
            weatherValues.add(wv.displayVal);
        }
        weatherAdapter = new ArrayAdapter<String>(this,
                android.R.layout.simple_spinner_item, weatherValues);
        weatherAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        mWeatherInput.setAdapter(weatherAdapter);
        mWeatherInput.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                Drawable drawable = MainActivity.this.getResources().getDrawable(MainActivity.this.getResources().getIdentifier(WeatherValues.getValue(adapterView.getSelectedItem().toString()).replace('-', '_'), "drawable", MainActivity.this.getPackageName()), MainActivity.this.getTheme());
                        ...
                        .addCallback(new BaseTransientBottomBar.BaseCallback<Snackbar>() {
                            ...
                            public void onDismissed(Snackbar transientBottomBar, int event) {
                                ...
                                if (event == Snackbar.Callback.DISMISS_EVENT_TIMEOUT) {
                                    ...
                                    toSend += " - " + WeatherValues.getValue(mWeatherInput.getSelectedItem().toString());
                                    toSend += " - eat";
